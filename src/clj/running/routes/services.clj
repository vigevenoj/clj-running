    ...
    (coerce/safe
     (fn [x]
       (if (and (string? x)
         ...
         db/string-duration-to-duration
         x)))))
   ...
   (context "/api/v1" []
            ...
            (context "/running" []
                     ...
                     (GET "/filter" [] ; todo filtering can be done on the /api/v1/runnning/runs endpoint
                          :query-params
                                   [{before :- (describe String "Runs before this date") ""}
                     ...
                     (context "/runs" []
                              ...
                              (POST "/" []
                                    ...
                                    (ok run))
                              ; todo: actually save this to the db!
                              (PUT "/" []
                                   :body         [run runs/Run]
                     ...
                     (context "/statistics" []
                              ...
                              (GET "/current/:period" []
                                   ...
                                   (runs/current-period-distance period units))
                              ; this just returns a response with total distance
                              (GET "/rolling/:period" []
